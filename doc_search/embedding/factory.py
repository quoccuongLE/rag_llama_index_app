...
_REGISTERED_EMBED_MODEL_CLS = {}
_REGISTERED_EMBED_MODEL_CFG = {}
...
def register_builder(key: str):
    ...
    """
    return registry.register(_REGISTERED_EMBED_MODEL_CLS, key)


def build(config: ConfigParams, name: str | None = None, **kwargs):
    builder = registry.lookup(_REGISTERED_EMBED_MODEL_CLS, name or config.type)
    return builder(config=config, **kwargs)


def get_config(name: str):
    """Looks up the `Config` according to the `name`."""
    cfg_creater = registry.lookup(_REGISTERED_EMBED_MODEL_CFG, name)
    return cfg_creater()


def register_config(key: str):
    return registry.register(_REGISTERED_EMBED_MODEL_CFG, key)
